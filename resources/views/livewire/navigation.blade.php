<nav class="bg-gray-800" x-data="{ open: false }">
        ...
        <div class="flex items-center">
          ...
          <div class="hidden md:block">
            <div class="ml-10 flex items-baseline space-x-4">
              ...
              <a href="{{route('posts.carreer', $carreer)}}" class="rounded-md px-3 py-2 text-sm font-medium text-gray-300 hover:bg-gray-700 hover:text-white">{{ $carreer->abbreviates }}</a>
              @endforeach
              ...
              <div class="relative ml-3" x-data="{ open: false }">
                ...
                <div x-show="open" @click.away="open = false" class="absolute right-0 z-10 mt-2 w-48 origin-top-right rounded-md bg-white py-1 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none" role="menu" aria-orientation="vertical" aria-labelledby="user-menu-button" tabindex="-1">
                  ...
                  <form action="{{ route('logout') }}" method="post">
                    @csrf
                    <button type="submit" class="block w-full text-left px-4 py-2 text-sm text-gray-700" role="menuitem" tabindex="-1" id="user-menu-item-1">Cerrar Sesión</button>
                  </form>
    ...
    <div x-show='open' x-on:click.away="open = false" class="md:hidden" id="mobile-menu">
      <div class="space-y-1 px-2 pb-3 pt-2 sm:px-3">
        ...
        @foreach ($carreers as $carreer)
          <a href="{{route('posts.carreer', $carreer)}}" class="block rounded-md px-3 py-2 text-base font-medium text-gray-300 hover:bg-gray-700 hover:text-white">{{ $carreer->abbreviates }}</a>
        @endforeach
      </div>
      ...
      <div class="border-t border-gray-700 pb-3 pt-4">
        @auth
          ...
          <div class="mt-3 space-y-1 px-2">
            ...
            <form action="{{ route('logout') }}" method="post">
              @csrf
              <button type="submit" class="block rounded-md px-3 py-2 text-base font-medium text-gray-400 hover:bg-gray-700 hover:text-white" role="menuitem" tabindex="-1" id="user-menu-item-2">Cerrar Sesión</button>
            </form>
          </div>
        @else
          <div class="px-2">
            <a href="{{route('login')}}" class="block w-full rounded-md px-3 py-2 text-base font-medium text-gray-400 hover:bg-gray-700 hover:text-white">Iniciar Sesión</a>
          </div>
        @endauth
      </div>
